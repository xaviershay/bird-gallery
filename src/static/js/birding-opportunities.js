...
async function encodeSnapshot(observations, region, location) {
    ...
    console.log(
        `[Snapshot] JSON: ${jsonSize} bytes → Gzipped: ${compressedSize} bytes (${ratio}% reduction)`,
    );
...
async function decodeSnapshot(encoded) {
    try {
        ...
        (decoded.l || []).forEach((loc) => {
            (loc.s || []).forEach(
                ([comName, speciesCode, obsDt, subId, tagMask]) => {
                    observations.push({
                        locName: loc.n,
                        locId: loc.i,
                        commonName: comName,
        ...
        console.error(
            "[Birding Opportunities] Failed to decode snapshot:",
            error,
        );
        return null;
    }
...
function getDisplayTags(tags) {
    ...
    if (tags.isLifer) {
        ...
        if (tags.isLocationLifer) {
            result.push({
                class: "tag-location-lifer",
                icon: "📍",
                title: "Location Lifer",
            });
        }
    }
...
async function initBirdingOpportunities() {
    ...
    console.log(
        "[Birding Opportunities] Initializing with region/location:",
        regionOrLocation,
    );
    ...
    if (snapshotData) {
        ...
        if (snapshot) {
            ...
            showStatus(
                `Showing snapshot from ${snapshot.date} (${allObservations.length} observations)`,
            );
    ...
    try {
        ...
        if (!apiKey) {
            console.log(
                "[Birding Opportunities] No API key found in localStorage",
            );
            showStatus("eBird API key required. Please enter your API key.");
            apiKey = promptForApiKey();
            if (!apiKey) {
                console.error(
                    "[Birding Opportunities] User cancelled API key prompt",
                );
                showError(
                    "eBird API key is required to fetch birding opportunities.",
                );
                return;
            }
            ...
            console.log(
                "[Birding Opportunities] Using existing API key from localStorage",
            );
        }
        ...
        console.log(
            "[Birding Opportunities] Fetching species tags from server...",
        );
        ...
        console.log(
            `[Birding Opportunities] Species tags loaded: ${tagsArray.length} species`,
        );

        // Fetch recent observations from eBird
        showStatus(`Fetching recent observations in ${regionOrLocation}...`);
        console.log(
            `[Birding Opportunities] Fetching recent observations from eBird API for ${regionOrLocation}, last ${DAYS_BACK} days...`,
        );
        ...
        console.log(
            `[Birding Opportunities] Received ${recentObs.length} recent observations from eBird`,
            recentObs,
        );
        ...
        recentObs.forEach((obs) => {
            ...
            // Species absent from personal list = true lifer (never seen anywhere)
            const tags = speciesTags[speciesCode] || {
                isLifer: true,
                isPhotoLifer: false,
                isYearLifer: true,
                isLocationLifer: true,
            };
        ...
        console.log(
            `[Birding Opportunities] Found ${targetSpecies.length} target species with tags:`,
            targetSpecies,
        );
        ...
        if (targetSpecies.length === 0) {
            ...
            showStatus(
                "No target birds found! Nothing recently reported matches your lifer criteria.",
            );
            renderResults([]);
            return;
        }
        ...
        showStatus(
            `Found ${targetSpecies.length} target species. Fetching locations...`,
        );
        console.log(
            `[Birding Opportunities] Fetching detailed observations for ${targetSpecies.length} species...`,
        );

        // Fetch detailed observations for each target species
        const observationPromises = targetSpecies.map(
            async (species, index) => {
                try {
                    // Check if data is cached before adding delay
                    const isCached = isSpeciesObservationsCached(
                        regionOrLocation,
                        species.code,
                        DAYS_BACK,
                    );
                    ...
                    console.log(
                        `[Birding Opportunities] Fetching observations for ${species.name} (${species.code})...`,
                    );
                    const obs = await getSpeciesObservations(
                        regionOrLocation,
                        species.code,
                        apiKey,
                        DAYS_BACK,
                    ...
                    console.log(
                        `[Birding Opportunities] Received ${obs.length} observations for ${species.name}`,
                    );
                    return obs.map((o) => ({
                        ...o,
                    ...
                    console.error(
                        `[Birding Opportunities] Failed to fetch observations for ${species.name}:`,
                        error,
                    );
                    return [];
                }
        ...
        console.log(
            `[Birding Opportunities] Total observations collected: ${allObservations.length}`,
            allObservations,
        );

        showStatus(
            `Found ${allObservations.length} observations of ${targetSpecies.length} species`,
        );
        ...
        console.error(
            "[Birding Opportunities] Error during initialization:",
            error,
        );
...
async function fetchSpeciesTags(region, location) {
    ...
    if (!response.ok) {
        console.error(
            "[Birding Opportunities] Failed to fetch species tags:",
            response.status,
            response.statusText,
        );
    ...
    console.log(
        "[Birding Opportunities] Species tags fetched successfully:",
        data.length,
        "species",
    );
    return data;
...
function renderResults(observations) {
    console.log(
        `[Birding Opportunities] Rendering results table with ${observations.length} observations`,
    );
    ...
    console.log(
        `[Birding Opportunities] Rendering ${locations.length} locations in table`,
    );
    ...
    if (countElem) {
        const uniqueSpeciesCount = new Set(
            observations.map((o) => o.speciesCode),
        ).size;
...
function renderMap(observations) {
    console.log(
        `[Birding Opportunities] Rendering map with ${observations.length} observations`,
    );
    if (!window.mapboxgl) {
    ...
    console.log(
        "[Birding Opportunities] GeoJSON created with",
        features.length,
        "features",
    );
    ...
    if (mapInstance) {
        ...
        if (source) {
            ...
            console.log(
                "[Birding Opportunities] Updated existing map data source",
            );
            return;
        }
    }
    ...
    console.log(
        "[Birding Opportunities] Initializing Mapbox map centered at",
        DEFAULT_MAP_CENTER,
    );
    ...
    mapInstance.on("load", () => {
        console.log(
            "[Birding Opportunities] Map loaded, adding data source and layers...",
        );
        ...
        console.log(
            "[Birding Opportunities] Map setup complete with all layers and event handlers",
        );
    });
}
...
function showShareButton() {
    const shareButtonContainer = document.getElementById(
...
async function copyShareLink() {
    try {
        ...
        const filtered = allObservations.filter((obs) => {
            ...
            if (tags.isPhotoLifer && !tags.isLifer && tagFilters["photo-lifer"])
                return true;
            if (tags.isYearLifer && !tags.isLifer && tagFilters["year-lifer"])
                return true;
            if (
                tags.isLocationLifer &&
                !tags.isLifer &&
                !tags.isYearLifer &&
                tagFilters["location-lifer"]
            )
                return true;
        ...
        console.error(
            "[Birding Opportunities] Failed to copy share link:",
            error,
        );
...
function applyTagFilters() {
    ...
    showStatus(
        `Showing ${filtered.length} observations of ${uniqueSpeciesCount} species`,
    );
}
