...
export async function handleReport(
  ...
  switch (reportId) {
    case "nophotos":
      ...
      const species = await fetchSpeciesMissingPhotos(env, region);
      let regionCount: Record<string, number> = {};
      const regions = ['', 'au-vic'];
      ...
      const counts = await Promise.all(regions.map(async region => {
        const local = await fetchSpeciesMissingPhotos(env, region);
        return ['' + region, local.length] as const;
      }));
      
      regionCount = Object.fromEntries(counts);

      const title = "Missing Photos - Xavier's Bird Lists";
      const content = MissingPhotosView({ species, region, stats: { regionCount } });
